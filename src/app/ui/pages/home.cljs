...
(defclassified HomeLink :a "hover:font-bold hover:text-black w-full cursor-pointer md:w-1/2 py-2 my-2 md:my-0 rounded-md text-center bg-gradient-to-r from-red-700 to-red-900")

(defnc HomeRenderer [_]
  ($ HomeWrapper
     ($ Navbar)
     (d/div {:class "bg-black text-white"}
            ...
            (d/div {:class "md:px-20 lg:px-80"}
                   (d/div {:class "mx-10 md:mx-0 text-left"}
                          (d/h5 {:class "text-xl md:text-2xl font-bold leading-relaxed"}
                          ...
                          (d/div {:class "flex flex-wrap mt-8 leading-relaxed text-sm md:text-base"}
                   ...
                   (d/div {:class "flex flex-col md:flex-row justify-center md:space-x-4 mt-4 mx-10 md:mx-0"}
                          ($ HomeLink {:href (router/get-url _ :router {:page "media"})} "Media")
                          ($ HomeLink {:href (router/get-url _ :router {:page "faq"})} "ReMind FAQ")))
            (d/div {:class "py-10 md:px-20 lg:px-80"}
                   (d/div {:class "flex flex-col place-items-center m-auto"}
                          (d/a {:href (router/get-url _ :router {:page "team"}) :class "pb-10 transform transition ease-in-out duration-500 hover:scale-125"}
                               (d/img {:src "../images/scylla-logo.jpeg" :class "w-48 h-48 md:w-64 md:h-64"}))
